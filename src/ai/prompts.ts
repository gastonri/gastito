  ...
  static buildTextPrompt(
    ...
    if (operacionPendiente) {
      ...
      contextoTexto =
        ...
        `Hay un GASTO pendiente que el usuario aún NO ha confirmado:\n` +
        `- Descripción: ${d.descripcion || "(pendiente)"}\n` +
        `- Monto: $${d.monto || 0} ${d.moneda || "ARS"}\n` +
        `- Fecha: ${d.fecha || "Hoy"}\n` +
        `- Categoría: ${d.macro_categoria || "(pendiente)"} → ${d.subcategoria || "(pendiente)"}\n` +
        `- Mi parte: ${d.mi_parte !== undefined ? d.mi_parte : 100}%\n` +
        `\n🎯 IMPORTANTE: El usuario está MODIFICANDO esta operación pendiente. ` +
        `MANTENÉ todos los campos que el usuario NO menciona. ` +
        `Si el mensaje es claramente un NUEVO gasto, ignorá esta operación pendiente.`;
    } else if (contextoPrevio) {
      const d = contextoPrevio.datos;
      contextoTexto =
        `\n\n📋 CONTEXTO DE LA CONVERSACIÓN ANTERIOR:\n` +
        `El usuario acaba de confirmar un GASTO:\n` +
        `- Descripción: ${d.descripcion}\n` +
        `- Monto: $${d.monto} ${d.moneda || "ARS"}\n` +
        `- Fecha: ${d.fecha || "Hoy"}\n` +
        `- Categoría: ${d.macro_categoria} → ${d.subcategoria}\n` +
